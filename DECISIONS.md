...
| 73f | The sweep's tabs **follow the named month**, and are chronological | `next` is only ever the forward slot, so a `--month` already forward of this one makes it redundant: sweeping November in September wants September and November, and October is a month you did not come here to think about. Sweeping this month still gets one, because there the forward slot is the whole point | live |
| 73g | You always open on **this month**, whichever was named | It is the one you are living in, and the sweep is about deciding what comes into it | live |
| 73d | `>` offers **every tab on screen** | Anything you can see is somewhere you can send a line. The closing month is a tab during a sweep and was not a destination, so a line could be carried out of it and never back — reachable only by quitting and re-running with a different `--month` | live |
| 73e | Beyond the tabs the two screens want **opposite things** | The sweep is closed to the months it opened: a fifth one in the picker is a month you did not come here to think about. The daily screen has two tabs, so `>` is the whole of how someday and next month are reached at all — which is what 10 rests on | live |

| 73c | The **sweep gets its own footer**: `> move to`, `t take`, and not add, tag or filter | It is one job — decide where each leftover goes — and the keys that job is made of are the two that move a line. The daily keys still work and are still in `?`; they are simply not what you came here to do. Same rule as 92h and 99d: a footer names what belongs to the screen, not everything that is bound | live |
