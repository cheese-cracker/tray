...
| 86m | The dialog **grows with the terminal**, capped at what the content can use | Fixed at its content's size it read as a tooltip on a big screen; uncapped it became a vast box with a paragraph adrift in it. The block inside is indented as one — `lipgloss.Place` centres each line separately, which scatters the key columns | live |
