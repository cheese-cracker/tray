...
| 86f | It opens by saying **what tray is** — four lines, before the diagram | The diagram answers "why two layers"; nothing answered "what am I even looking at". It explains the idea — dump into the month you expect to pick it up, take it onto the tray when you are ready — not the file layout, which nobody needs on first read | live |
| 86h | The key is written **inside the verb**: `(a)dd`, `(t)ake` | It reads as a word and marks the key at once. The keys table leaves them bare, where a column already says what they are | live |
| 86l | **The diagram carries no keys.** They sit under it as their own lines | A picture carrying its own keybindings is doing two jobs and neither well. It now says only what the two layers are; the four moves are read as a list, which is how a list is read | live |
| 86j | `(d)` stays bare — no d-word to sit in | "hand back" has no `d`, and calling it "drop" would collide with `D`, which already writes `dropped:`. Better a bare letter than a name that means something else | live |
...
| 86i | The boxes carry their **name in the top edge** | It buys back the row a name-as-first-line costs, and the dialog has to fit twenty rows to stay clear of the footer | live |
