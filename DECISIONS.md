...
| 86h | Letters in the diagram are **parenthesised**: `(a)` `(t)` `(d)` | Inline in prose a bare letter reads as a word, and `a` means a different thing in each box. The keys section leaves them bare, where a dedicated column already says what they are | live |
